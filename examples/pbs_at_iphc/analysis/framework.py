...
"""
Law example tasks to demonstrate Pbs workflows at the Desy Maxwell cluster.
...
# the pbs workflow implementation is part of a law contrib package
# so we need to explicitly load it
law.contrib.load("pbs")
...
class PbsWorkflow(law.pbs.PbsWorkflow):
    """
    Batch systems are typically very heterogeneous by design, and so is Pbs. Law does not aim
    to "magically" adapt to all possible Pbs setups which would certainly end in a mess.
    Therefore we have to configure the base Pbs workflow in law.contrib.pbs to work with
